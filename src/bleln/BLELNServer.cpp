    ...
    */
    ...
    bool BLELNServer::sendEncrypted(uint16_t h, const std::string &msg) {
        ...
        if (xQueueSend(workerActionQueue, &pkt, pdMS_TO_TICKS(200)) != pdPASS) {
            free(heapBuf);
            return false;
        }
    ...
    void BLELNServer::worker_sendMessage(uint16_t h, uint8_t *data, size_t dataLen) {
        Serial.println("worker_sendMessage");
        if(h==UINT16_MAX){
            Serial.println("sendToAll");
